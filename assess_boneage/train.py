...
parser.add_argument('--lr','--learning-rate',default=0.000063,type=float,metavar='LR',help='initial learning rate') #0.000125
...
parser.add_argument('--resume',default='/content/checkpoints/resume/Assess_BoneAge_InceptionV3_SE_PAM_CAM_NO_SEX_43.pth.tar',type=str,metavar='PATH',help='path to latest checkpoint(default:None)')#/content/checkpoints/resume/Assess_BoneAge_InceptionV3_4.pth.tar
...
def main():
    ...
    trainset = AgeDataset(csv_file="/content/dataset/train.csv",transform=transform_train,root_dir='/content/dataset/train_no_adjust')
    trainloader = data.DataLoader(trainset,batch_size=args.train_batch,shuffle=True,num_workers=args.workers)
    testset = AgeDataset(csv_file='/content/dataset/valid.csv',transform=transform_test,root_dir='/content/dataset/valid_no_adjust')
...
#     scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, 'min')
    ...
    title = 'Assess_BoneAge_InceptionV3_SE_PAM_CAM_NO_SEX'
...
#         state['lr'] = optimizer.param_groups[0]['lr']
        ...
        if (epoch+1)%5 == 0 or test_loss<6.8:
            save_checkpoint({
                'epoch':epoch+1,
